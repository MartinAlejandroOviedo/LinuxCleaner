...
state_dir=$(mktemp -d)
trap 'find "$build_dir" "$repo_dir" "$state_dir" -depth -delete 2>/dev/null || true' EXIT
...
flatpak-builder --force-clean --state-dir="$state_dir" --default-branch=stable --repo="$repo_dir" "$build_dir" \
